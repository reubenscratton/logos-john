// English glosses by lemma. Extend when a new chapter adds vocabulary.
export const GLOSS_EN = {
  "Βηθανία": "Bethany",
  "Βηθσαϊδά": "Bethsaida",
  "Γαλιλαία": "Galilee",
  "Κανά": "Cana",
  "Κηφᾶς": "Cephas",
  "Λευίτης": "Levite",
  "Μεσσίας": "Messiah",
  "Μωϋσῆς": "Moses",
  "Ναζαρά": "Nazareth",
  "Ναθαναήλ": "Nathanael",
  "Πέτρος": "Peter",
  "Σίμων": "Simon",
  "Φίλιππος": "Philip",
  "Φαρισαῖος": "Pharisee",
  "Χριστός": "Christ, Anointed",
  "αἴρω": "to take away, lift up",
  "αἷμα": "blood",
  "αὐτός": "he, it, same",
  "βαπτίζω": "to baptize",
  "βασιλεύς": "king",
  "βλέπω": "to see, look",
  "βοάω": "to cry out",
  "γάμος": "wedding",
  "γίνομαι": "to become, come to be",
  "γεμίζω": "to fill",
  "γεννάω": "to beget, give birth",
  "γεύομαι": "to taste",
  "γινώσκω": "to know",
  "γράφω": "to write",
  "γυνή": "woman",
  "δέ": "but, and",
  "δέκατος": "tenth",
  "δίδωμι": "to give",
  "διά": "through, on account of",
  "διάκονος": "servant",
  "διδάσκαλος": "teacher",
  "δόλος": "deceit, guile",
  "δόξα": "glory",
  "δύναμαι": "to be able",
  "δύο": "two",
  "εἰ": "if",
  "εἰμί": "to be",
  "εἰς": "into, to",
  "εἷς": "one",
  "εὐθύνω": "to make straight",
  "εὑρίσκω": "to find",
  "ζητέω": "to seek",
  "ζωή": "life",
  "θέλημα": "will",
  "θέλω": "to wish, will",
  "θεάομαι": "to behold, gaze at",
  "θεός": "God",
  "καί": "and",
  "καθαρισμός": "purification",
  "καθώς": "just as",
  "καλέω": "to call",
  "καλός": "good, fine",
  "κατά": "according to, against",
  "καταβαίνω": "to come down",
  "καταλαμβάνω": "seized / grasped",
  "κεῖμαι": "to lie, be placed",
  "κράζω": "to cry out",
  "κόλπος": "bosom, side",
  "κόσμος": "world",
  "κύριος": "lord",
  "κἀγώ": "and I, I too",
  "λέγω": "to say",
  "λίθινος": "of stone",
  "λαλέω": "to speak",
  "λαμβάνω": "to take, receive",
  "λόγος": "word",
  "λύω": "to loose, untie",
  "μέγας": "great",
  "μένω": "to remain, abide",
  "μέσος": "middle, midst",
  "μήτηρ": "mother",
  "μαθητής": "disciple",
  "μαρτυρέω": "to bear witness",
  "μαρτυρία": "testimony",
  "μεθερμηνεύω": "to translate",
  "μεθύω": "to be drunk",
  "μετρητής": "measure (metretes)",
  "μονογενής": "only, only-begotten",
  "νυμφίος": "bridegroom",
  "νόμος": "law",
  "νῦν": "now",
  "οἶδα": "to know",
  "οἶνος": "wine",
  "οὐ": "not",
  "οὐδέ": "and not, nor",
  "οὐδείς": "no one",
  "οὐρανός": "heaven, sky",
  "οὔπω": "not yet",
  "οὖν": "therefore, then",
  "οὗτος": "this one",
  "πάλιν": "again",
  "πέμπω": "to send",
  "πέραν": "beyond, across",
  "παρά": "from, beside",
  "παραλαμβάνω": "to receive, take",
  "πατήρ": "father",
  "περί": "concerning, about",
  "περιπατέω": "to walk",
  "περιστερά": "dove",
  "πιστεύω": "to believe, trust",
  "πλήρης": "full",
  "πλήρωμα": "fullness",
  "πνεῦμα": "spirit, breath",
  "ποιέω": "to do, make",
  "πού": "somewhere",
  "ποῦ": "where?",
  "προφήτης": "prophet",
  "πρό": "before",
  "πρός": "to, toward, with",
  "πρῶτος": "first",
  "πόθεν": "from where",
  "πόλις": "city",
  "πώποτε": "ever, at any time",
  "πᾶς": "all, every",
  "σάρξ": "flesh",
  "σεαυτοῦ": "yourself",
  "σημεῖον": "sign",
  "σκηνόω": "to pitch a tent, dwell",
  "σκοτία": "darkness",
  "στήκω": "to stand",
  "στρέφω": "to turn",
  "συκῆ": "fig tree",
  "σύ": "you",
  "τέκνον": "child",
  "τίθημι": "to put, place",
  "τίς": "who? what?",
  "τηρέω": "to keep",
  "τρίτος": "third",
  "τρεῖς": "three",
  "τὶς": "someone, a certain",
  "υἱός": "son",
  "φέρω": "to carry, bring",
  "φαίνω": "shines",
  "φανερόω": "to reveal",
  "φημί": "to say",
  "φωνέω": "to call",
  "φωνή": "voice, sound",
  "φωτίζω": "to give light to",
  "φῶς": "light",
  "χάρις": "grace, favor",
  "χωρέω": "to hold, contain",
  "χωρίς": "apart from",
  "ἀγαθός": "good",
  "ἀδελφός": "brother",
  "ἀκολουθέω": "to follow",
  "ἀκούω": "to hear",
  "ἀλήθεια": "truth",
  "ἀληθινός": "true, genuine",
  "ἀληθῶς": "truly",
  "ἀλλά": "but",
  "ἀμήν": "amen, truly",
  "ἀμνός": "lamb",
  "ἀνά": "each, apiece",
  "ἀνήρ": "man, husband",
  "ἀναβαίνω": "to go up",
  "ἀνοίγω": "to open",
  "ἀντί": "instead of, for",
  "ἀντλέω": "to draw (water)",
  "ἀποκρίνομαι": "to answer",
  "ἀποστέλλω": "to send (out)",
  "ἀπό": "from",
  "ἀπόκρισις": "answer",
  "ἀρνέομαι": "to deny",
  "ἀρχή": "beginning",
  "ἀρχιτρίκλινος": "master of the feast",
  "ἁμαρτία": "sin",
  "ἄγγελος": "angel, messenger",
  "ἄγω": "to lead, bring",
  "ἄν": "(untranslatable particle)",
  "ἄνθρωπος": "human being",
  "ἄνω": "up, to the brim",
  "ἄξιος": "worthy",
  "ἄρτι": "now, just now",
  "ἅγιος": "holy",
  "Ἀνδρέας": "Andrew",
  "ἐγώ": "I",
  "ἐκ": "out of, from",
  "ἐκεῖ": "there",
  "ἐκεῖνος": "that one",
  "ἐλάσσων": "lesser, inferior",
  "ἐμβλέπω": "to look at",
  "ἐν": "in",
  "ἐξέρχομαι": "to go out",
  "ἐξηγέομαι": "to explain, make known",
  "ἐξουσία": "authority, right",
  "ἐπί": "upon, on",
  "ἐπαύριον": "the next day",
  "ἐρωτάω": "to ask",
  "ἑρμηνεύω": "to translate, interpret",
  "ἔμπροσθεν": "before, ahead of",
  "ἔρημος": "wilderness, desert",
  "ἔρχομαι": "to come, go",
  "ἔχω": "to have",
  "ἕξ": "six",
  "ἕως": "until",
  "ἡμέρα": "day",
  "ἤ": "or",
  "ἥκω": "to have come",
  "Ἠλίας": "Elijah",
  "Ἠσαΐας": "Isaiah",
  "ἱερεύς": "priest",
  "ἱμάς": "strap, thong",
  "ἴδιος": "one’s own",
  "ἵνα": "so that, in order that",
  "ἵστημι": "to stand",
  "Ἰησοῦς": "Jesus",
  "Ἰορδάνης": "Jordan",
  "Ἰουδαῖος": "Judean, Jew",
  "Ἰσραήλ": "Israel",
  "Ἰσραηλίτης": "Israelite",
  "Ἰωάννης": "John",
  "Ἰωσήφ": "Joseph",
  "Ἱεροσόλυμα": "Jerusalem",
  "ὀπίσω": "after, behind",
  "ὁ": "the",
  "ὁδός": "way, road",
  "ὁμολογέω": "to confess, acknowledge",
  "ὁράω": "to see",
  "ὄνομα": "name",
  "ὅπου": "where",
  "ὅς": "who, which",
  "ὅσος": "as many as",
  "ὅταν": "whenever",
  "ὅτε": "when",
  "ὅτι": "that, because",
  "ὑδρία": "water-jar",
  "ὑπέρ": "above, on behalf of",
  "ὑποκάτω": "under, beneath",
  "ὑπό": "under, by",
  "ὑπόδημα": "sandal",
  "ὑστερέω": "to lack, run short",
  "ὕδωρ": "water",
  "ὡς": "as, like",
  "ὥρα": "hour",
  "ῥαββί": "rabbi",
  "μετά": "with, after", "Καφαρναούμ": "Capernaum", "πολύς": "much, many", "ἐγγύς": "near",
  "πάσχα": "Passover", "ἱερός": "sacred; temple", "πωλέω": "to sell", "βοῦς": "ox, cattle",
  "πρόβατον": "sheep", "κερματιστής": "money-changer", "κάθημαι": "to sit", "φραγέλλιον": "whip",
  "σχοινίον": "cord, rope", "ἐκβάλλω": "to drive out", "τέ": "and, both", "κολλυβιστής": "money-changer",
  "ἐκχέω": "to pour out", "κέρμα": "coin", "τράπεζα": "table", "ἀνατρέπω": "to overturn",
  "ἐντεῦθεν": "from here", "μή": "not", "οἶκος": "house", "ἐμπόριον": "house of trade",
  "μιμνῄσκω": "to remember", "ζῆλος": "zeal", "κατεσθίω": "to consume, devour", "δείκνυμι": "to show",
  "ναός": "temple, sanctuary", "ἐγείρω": "to raise", "τεσσεράκοντα": "forty", "ἔτος": "year",
  "οἰκοδομέω": "to build", "σῶμα": "body", "νεκρός": "dead", "γραφή": "Scripture", "ἑορτή": "feast",
  "θεωρέω": "to observe, behold", "ἑαυτοῦ": "himself", "χρεία": "need", "γάρ": "for, because",
  // Chapter 3
  "Νικόδημος": "Nicodemus", "ἄρχων": "ruler", "νύξ": "night", "ἐάν": "if",
  "ἄνωθεν": "from above; again", "βασιλεία": "kingdom", "πῶς": "how", "γέρων": "old man",
  "κοιλία": "womb, belly", "δεύτερος": "second", "εἰσέρχομαι": "to enter", "θαυμάζω": "to marvel, be amazed",
  "δεῖ": "it is necessary, must", "πνέω": "to blow", "ὑπάγω": "to go, depart", "οὕτω": "thus, in this way",
  "ἐπίγειος": "earthly", "ἐπουράνιος": "heavenly", "ὑψόω": "to lift up, exalt", "ὄφις": "serpent",
  "αἰώνιος": "eternal", "ἀγαπάω": "to love", "ὥστε": "so that", "ἀπόλλυμι": "to perish, destroy",
  "κρίνω": "to judge, condemn", "σῴζω": "to save", "ἤδη": "already, now", "κρίσις": "judgment",
  "μᾶλλον": "rather, more", "σκότος": "darkness", "πονηρός": "evil, wicked", "ἔργον": "work, deed",
  "φαῦλος": "worthless, wicked", "πράσσω": "to do, practise", "μισέω": "to hate", "ἐλέγχω": "to expose, convict",
  "ἐργάζομαι": "to work, do", "γῆ": "earth, land", "διατρίβω": "to stay, remain", "Αἰνών": "Aenon",
  "Σαλείμ": "Salim", "παραγίνομαι": "to come, arrive", "βάλλω": "to throw, cast", "φυλακή": "prison, guard",
  "ζήτησις": "dispute, discussion", "νύμφη": "bride", "φίλος": "friend", "χαρά": "joy",
  "χαίρω": "to rejoice", "ἐμός": "my, mine", "πληρόω": "to fill, fulfil", "αὐξάνω": "to increase, grow",
  "ἐλαττόω": "to decrease, grow less", "ἐπάνω": "above, over", "σφραγίζω": "to seal, certify", "ἀληθής": "true",
  "ῥῆμα": "word, saying", "μέτρον": "measure", "χείρ": "hand", "ἀπειθέω": "to disobey, refuse to believe",
  "ὀργή": "wrath, anger",
  // Chapter 4
  "καίτοιγε": "although, and yet", "ἀφίημι": "to leave, let go", "Ἰουδαία": "Judea", "ἀπέρχομαι": "to go away",
  "διέρχομαι": "to pass through", "Σαμάρεια": "Samaria", "Συχάρ": "Sychar", "πλησίον": "near",
  "χωρίον": "plot of land, field", "Ἰακώβ": "Jacob", "πηγή": "spring, well", "κοπιάω": "to grow weary, toil",
  "ὁδοιπορία": "journey", "καθέζομαι": "to sit down", "ἕκτος": "sixth", "πίνω": "to drink",
  "τροφή": "food", "ἀγοράζω": "to buy", "Σαμαρῖτις": "Samaritan woman", "αἰτέω": "to ask, request",
  "συγχράομαι": "to associate with", "Σαμαρίτης": "Samaritan", "δωρεά": "gift", "ζάω": "to live",
  "οὔτε": "neither, nor", "ἄντλημα": "bucket", "φρέαρ": "well, cistern", "βαθύς": "deep",
  "θρέμμα": "livestock", "διψάω": "to thirst", "αἰών": "age; forever", "ἅλλομαι": "to leap, well up",
  "μηδέ": "nor, not even", "ἐνθάδε": "here", "καλῶς": "well, rightly", "πέντε": "five",
  "εἶπον": "to say (aor. of λέγω)", "ὄρος": "mountain", "προσκυνέω": "to worship, bow down", "τόπος": "place",
  "σωτηρία": "salvation", "προσκυνητής": "worshiper", "τοιοῦτος": "such", "ἀναγγέλλω": "to announce, declare",
  "ἅπας": "all, everything", "μέντοι": "however, yet", "δεῦτε": "come!", "μήτι": "(question expecting “no”)",
  "μεταξύ": "between; meanwhile", "ἐσθίω": "to eat", "βρῶσις": "food, eating", "ἀλλήλων": "one another",
  "βρῶμα": "food", "τελειόω": "to complete, finish", "ἔτι": "still, yet", "τετράμηνος": "four months",
  "θερισμός": "harvest", "ἐπαίρω": "to lift up", "ὀφθαλμός": "eye", "χώρα": "field, region",
  "λευκός": "white", "θερίζω": "to reap", "μισθός": "wages, reward", "συνάγω": "to gather",
  "καρπός": "fruit", "σπείρω": "to sow", "ὁμοῦ": "together", "ἄλλος": "other, another",
  "κόπος": "labor, toil", "οὐκέτι": "no longer", "σός": "your", "λαλιά": "speech, telling",
  "σωτήρ": "savior", "ἐκεῖθεν": "from there", "πατρίς": "hometown", "τιμή": "honor",
  "δέχομαι": "to receive, welcome", "Γαλιλαῖος": "Galilean", "βασιλικός": "royal official", "ἀσθενέω": "to be sick",
  "ἰάομαι": "to heal", "μέλλω": "to be about to", "ἀποθνῄσκω": "to die", "τέρας": "wonder, portent",
  "πρίν": "before", "παιδίον": "child", "πορεύομαι": "to go, proceed", "δοῦλος (II)": "servant, slave",
  "ὑπαντάω": "to meet", "παῖς": "boy, servant", "πυνθάνομαι": "to inquire, ask", "κομψότερον": "better (in health)",
  "ἐχθές": "yesterday", "ἕβδομος": "seventh", "πυρετός": "fever", "οἰκία": "household, house", "ὅλος": "whole, all",
  // Chapter 5
  "προβατικός": "of sheep (Sheep Gate)", "κολυμβήθρα": "pool", "ἐπιλέγω": "to call, name", "Ἑβραϊστί": "in Hebrew/Aramaic",
  "Βηθζαθά": "Bethzatha", "στοά": "colonnade, porch", "κατάκειμαι": "to lie down", "πλῆθος": "multitude, crowd",
  "τυφλός": "blind", "χωλός": "lame", "ξηρός": "withered, paralysed", "ἐκδέχομαι": "to wait for, expect",
  "κίνησις": "moving, stirring", "καιρός": "appointed time, season", "ταράσσω": "to stir up, trouble", "ἐμβαίνω": "to step in",
  "ταραχή": "stirring, disturbance", "ὑγιής": "well, whole, healthy", "δήποτε": "whatever", "κατέχω": "to hold fast, afflict",
  "νόσημα": "disease", "τριάκοντα": "thirty", "ὀκτώ": "eight", "ἀσθένεια": "weakness, illness",
  "χρόνος": "time", "κράβαττος": "mat, pallet", "εὐθέως": "at once, immediately", "σάββατον": "Sabbath",
  "θεραπεύω": "to heal, cure", "ἔξεστι(ν)": "it is lawful", "ἐκνεύω": "to withdraw, slip away", "ὄχλος": "crowd",
  "μηκέτι": "no longer, no more", "ἁμαρτάνω": "to sin", "χείρων": "worse", "διώκω": "to pursue, persecute",
  "ἀποκτείνω": "to kill", "μόνος": "only, alone", "ἴσος": "equal", "ὁμοίως": "likewise",
  "φιλέω": "to love, befriend", "ὥσπερ": "just as", "ζῳοποιέω": "to give life", "τιμάω": "to honour",
  "μεταβαίνω": "to pass over, cross", "θάνατος": "death", "μνημεῖον": "tomb", "ἐκπορεύομαι": "to come out",
  "ἀνάστασις": "resurrection", "ἐμαυτοῦ": "myself", "δίκαιος": "just, righteous", "λύχνος": "lamp",
  "καίω": "to burn, kindle", "ἀγαλλιάω": "to rejoice, exult", "εἶδος": "form, appearance", "ἐραυνάω": "to search",
  "δοκέω": "to think, suppose", "ἀγάπη": "love", "κατηγορέω": "to accuse", "ἐλπίζω": "to hope", "γράμμα": "writing, letter",
  // Chapter 6
  "θάλασσα": "sea", "Τιβεριάς": "Tiberias", "ἀνέρχομαι": "to go up", "ἄρτος": "bread, loaf",
  "πειράζω": "to test, tempt", "διακόσιοι": "two hundred", "δηνάριον": "denarius", "ἀρκέω": "to suffice",
  "ἕκαστος": "each", "βραχύς": "little, short", "παιδάριον": "little boy", "ὧδε": "here",
  "κρίθινος": "of barley", "ὀψάριον": "fish", "τοσοῦτος": "so many, so great", "ἀναπίπτω": "to recline, sit down",
  "χόρτος": "grass", "ἀριθμός": "number", "πεντακισχίλιοι": "five thousand", "εὐχαριστέω": "to give thanks",
  "διαδίδωμι": "to distribute", "ἀνάκειμαι": "to recline (at table)", "ἐμπίπλημι": "to fill, satisfy", "περισσεύω": "to be left over, abound",
  "κλάσμα": "fragment, piece", "δώδεκα": "twelve", "κόφινος": "basket", "βιβρώσκω": "to eat",
  "ἁρπάζω": "to seize, snatch", "ἀναχωρέω": "to withdraw", "ὄψιος": "evening, late", "πλοῖον": "boat",
  "ἄνεμος": "wind", "διεγείρω": "to stir up, arouse", "ἐλαύνω": "to row, drive", "στάδιον": "stadion (~185 m)",
  "εἴκοσι": "twenty", "φοβέω": "to fear", "πλοιάριον": "small boat", "συνεισέρχομαι": "to enter with",
  "πότε": "when", "χορτάζω": "to fill, satisfy", "μάννα": "manna", "πάντοτε": "always",
  "πεινάω": "to hunger", "ἔξω": "out, outside", "ἀνίστημι": "to raise up", "ἔσχατος": "last",
  "γογγύζω": "to grumble, murmur", "ἕλκω": "to draw, drag", "διδακτός": "taught", "μανθάνω": "to learn",
  "μάχομαι": "to fight, quarrel", "τρώγω": "to gnaw, chew, eat", "πόσις": "drink", "κἀκεῖνος": "and he, that one also",
  "συναγωγή": "synagogue", "διδάσκω": "to teach", "σκληρός": "hard, harsh", "σκανδαλίζω": "to offend, cause to stumble",
  "πρότερος": "former, before", "ὠφελέω": "to profit, help", "παραδίδωμι": "to hand over, betray", "ἐκλέγω": "to choose",
  "διάβολος": "devil, slanderer", "Ἰούδας": "Judas", "Ἰσκαριώθ": "Iscariot",
  // Chapter 7
  "σκηνοπηγία": "Feast of Tabernacles", "κρυπτός": "hidden, secret", "παρρησία": "openness, boldness", "πάρειμι": "to be present, have come",
  "ὑμέτερος": "your", "ἕτοιμος": "ready", "τότε": "then", "φανερῶς": "openly",
  "γογγυσμός": "muttering, murmuring", "μέν": "on the one hand", "πλανάω": "to lead astray, deceive", "φόβος": "fear",
  "μεσόω": "to be in the middle", "διδαχή": "teaching", "πότερον": "whether", "ἀδικία": "unrighteousness, falsehood",
  "δαιμόνιον": "demon", "περιτομή": "circumcision", "περιτέμνω": "to circumcise", "χολάω": "to be angry",
  "ὄψις": "appearance, face", "Ἱεροσολυμίτης": "a person of Jerusalem", "πιάζω": "to seize, arrest", "ἐπιβάλλω": "to lay on",
  "ἀρχιερεύς": "chief priest", "ὑπηρέτης": "officer, attendant", "μικρός": "little, small", "διασπορά": "Dispersion, Diaspora",
  "Ἕλλην": "Greek", "ποταμός": "river", "ῥέω": "to flow", "οὐδέπω": "not yet",
  "δοξάζω": "to glorify", "σπέρμα": "seed, offspring", "Δαυίδ": "David", "Βηθλέεμ": "Bethlehem",
  "κώμη": "village", "σχίσμα": "division, split", "οὐδέποτε": "never", "ἐπάρατος": "accursed",
  // Chapter 8
  "ἐλαία": "olive (tree)", "ὄρθρος": "dawn, early morning", "λαός": "people", "καθίζω": "to sit down",
  "γραμματεύς": "scribe", "μοιχεία": "adultery", "ἐπαυτοφώρῳ": "in the very act", "μοιχεύω": "to commit adultery",
  "ἐντέλλομαι": "to command", "λιθοβολέω": "to stone", "κάτω": "down, below", "κύπτω": "to bend down, stoop",
  "δάκτυλος": "finger", "ἐπιμένω": "to persist, continue", "ἀνακύπτω": "to straighten up", "ἀναμάρτητος": "without sin",
  "λίθος": "stone", "συνείδησις": "conscience", "καθεῖς": "one by one", "ἄρχω": "to begin, rule",
  "πρεσβύτερος": "elder", "καταλείπω": "to leave behind", "μηδείς": "no one", "πλήν": "except",
  "κατήγορος": "accuser", "κατακρίνω": "to condemn", "κἄν": "even if, and if", "γαζοφυλάκιον": "treasury",
  "ἀρεστός": "pleasing", "ἐλευθερόω": "to set free", "Ἀβραάμ": "Abraham", "δουλεύω": "to serve, be a slave",
  "ἐλεύθερος": "free", "ὄντως": "really, indeed", "πορνεία": "sexual immorality", "ἐπιθυμία": "desire, lust",
  "ἀνθρωποκτόνος": "murderer", "ψεῦδος": "lie, falsehood", "ψεύστης": "liar", "ἀτιμάζω": "to dishonour",
  "ὅστις": "whoever, who", "ὅμοιος": "like, similar", "πεντήκοντα": "fifty", "κρύπτω": "to hide",
  // Chapter 9
  "παράγω": "to pass by", "γενετή": "birth", "γονεύς": "parent", "πτύω": "to spit",
  "χαμαί": "on the ground", "πηλός": "mud, clay", "πτύσμα": "saliva, spittle", "ἐπιτίθημι": "to put on, lay on",
  "νίπτω": "to wash", "Σιλωάμ": "Siloam", "γείτων": "neighbour", "προσαίτης": "beggar",
  "προσαιτέω": "to beg", "οὐχί": "no, not (emphatic)", "ἐπιχρίω": "to smear on, anoint", "ἀναβλέπω": "to receive sight, look up",
  "ποτέ": "once, formerly", "ἁμαρτωλός": "sinner, sinful", "ἡλικία": "age, maturity", "συντίθημι": "to agree, arrange",
  "ἀποσυνάγωγος": "put out of the synagogue", "ἐπερωτάω": "to ask, question", "λοιδορέω": "to revile, insult", "θαυμαστός": "amazing, marvellous",
  "θεοσεβής": "God-fearing, devout", "κρίμα": "judgement",
  // Chapter 10
  "θύρα": "door, gate", "αὐλή": "courtyard, fold", "ἀλλαχόθεν": "from another place", "κλέπτης": "thief",
  "λῃστής": "robber, bandit", "ποιμήν": "shepherd", "θυρωρός": "doorkeeper, gatekeeper", "ἐξάγω": "to lead out",
  "ἀλλότριος": "stranger, another's", "φεύγω": "to flee", "παροιμία": "figure of speech, proverb", "νομή": "pasture",
  "κλέπτω": "to steal", "θύω": "to kill, sacrifice", "περισσός": "abundant, more than enough", "ψυχή": "life, soul",
  "μισθωτός": "hired hand", "λύκος": "wolf", "σκορπίζω": "to scatter", "μέλει": "it is a care, it matters",
  "ποίμνη": "flock", "ἐντολή": "commandment, charge", "μαίνομαι": "to be out of one's mind", "δαιμονίζομαι": "to be demon-possessed",
  "ἐγκαίνια": "Feast of Dedication", "χειμών": "winter", "Σολομών": "Solomon", "κυκλόω": "to surround, encircle",
  "βαστάζω": "to pick up, carry", "λιθάζω": "to stone", "ποῖος": "what kind of, which", "βλασφημία": "blasphemy",
  "ἁγιάζω": "to consecrate, make holy", "βλασφημέω": "to blaspheme",
  // Chapter 11
  "Λάζαρος": "Lazarus", "Μαρία": "Mary", "Μάρθα": "Martha", "ἀδελφή": "sister",
  "ἀλείφω": "to anoint", "μύρον": "ointment, perfume", "ἐκμάσσω": "to wipe", "πούς": "foot",
  "θρίξ": "hair", "ἔπειτα": "then, afterward", "προσκόπτω": "to stumble", "κοιμάομαι": "to fall asleep, sleep",
  "ἐξυπνίζω": "to wake up", "κοίμησις": "sleep, repose", "ὕπνος": "sleep", "Θωμᾶς": "Thomas",
  "Δίδυμος": "Twin (Didymus)", "συμμαθητής": "fellow disciple", "τέσσαρες": "four", "δεκαπέντε": "fifteen",
  "παραμυθέομαι": "to console, comfort", "ναί": "yes", "λάθρᾳ": "secretly, quietly", "ταχύς": "quick",
  "ταχέως": "quickly", "κλαίω": "to weep", "πίπτω": "to fall", "αὐτοῦ": "there, in that place",
  "συνέρχομαι": "to come together, accompany", "ἐμβριμάομαι": "to be deeply moved, groan", "δακρύω": "to shed tears", "σπήλαιον": "cave",
  "ἐπίκειμαι": "to lie on, be placed against", "τελευτάω": "to die, come to an end", "ὄζω": "to give off an odor, stink", "τεταρταῖος": "on the fourth day",
  "περιΐστημι": "to stand around", "κραυγάζω": "to cry out", "δεῦρο": "come here!", "θνῄσκω": "to die",
  "δέω": "to bind", "κειρία": "grave-band, linen strip", "σουδάριον": "face-cloth, napkin", "περιδέω": "to bind around, wrap",
  "συνέδριον": "council, Sanhedrin", "Ῥωμαῖος": "Roman", "ἔθνος": "nation", "Καϊάφας": "Caiaphas",
  "ἐνιαυτός": "year", "λογίζομαι": "to reckon, consider", "συμφέρω": "to be better, be profitable", "προφητεύω": "to prophesy",
  "διασκορπίζω": "to scatter abroad", "βουλεύω": "to plan, resolve", "Ἐφραίμ": "Ephraim", "κἀκεῖ": "and there",
  "ἁγνίζω": "to purify", "μηνύω": "to report, disclose", "ὅπως": "so that, in order that",
  // Chapter 12
  "δεῖπνον": "dinner, supper", "διακονέω": "to serve, wait on", "σύν": "with", "λίτρα": "pound (Roman)",
  "νάρδος": "nard, spikenard", "πιστικός": "pure, genuine", "πολύτιμος": "very costly", "ὀσμή": "fragrance, odor",
  "πιπράσκω": "to sell", "τριακόσιοι": "three hundred", "πτωχός": "poor, beggar", "γλωσσόκομον": "moneybag, purse",
  "ἐνταφιασμός": "burial, preparation for burial", "βάϊον": "palm branch", "φοῖνιξ": "palm tree", "ὑπάντησις": "a meeting",
  "ὡσαννά": "Hosanna (save now!)", "εὐλογέω": "to bless", "ὀνάριον": "young donkey", "θυγάτηρ": "daughter",
  "Σιών": "Zion", "πῶλος": "colt, foal", "ὄνος": "donkey", "προσέρχομαι": "to come to, approach",
  "κόκκος": "grain, seed", "σῖτος": "wheat, grain", "φυλάσσω": "to guard, keep", "βροντή": "thunder",
  "σημαίνω": "to signify, indicate", "ἀκοή": "report, hearing, message", "βραχίων": "arm", "ἀποκαλύπτω": "to reveal",
  "τυφλόω": "to blind", "πωρόω": "to harden", "καρδία": "heart", "νοέω": "to understand, perceive",
  "ὅμως": "nevertheless, yet", "ἤπερ": "than", "ἀθετέω": "to reject, set aside",
  // Chapter 13
  "τέλος": "end, completion", "ἱμάτιον": "garment, cloak", "λέντιον": "towel", "διαζώννυμι": "to gird, tie around",
  "εἶτα": "then, next", "νιπτήρ": "basin", "μέρος": "part, share", "κεφαλή": "head",
  "λούω": "to bathe, wash", "καθαρός": "clean, pure", "ὀφείλω": "to owe, ought", "ὑπόδειγμα": "example, pattern",
  "ἀπόστολος": "one sent, apostle", "μακάριος": "blessed, happy", "πτέρνα": "heel", "ἀπορέω": "to be at a loss, perplexed",
  "νεύω": "to nod, motion", "στῆθος": "chest, breast", "βάπτω": "to dip", "ψωμίον": "morsel, piece of bread",
  "Σατανᾶς": "Satan", "ἐπεί": "since, because", "τεκνίον": "little child", "καινός": "new",
  "ὕστερος": "later, afterward", "ἀλέκτωρ": "rooster, cock", "τρίς": "three times",
  // Chapter 14
  "μονή": "dwelling place, room", "ἑτοιμάζω": "to prepare", "παράκλητος": "Helper, Advocate, Comforter", "ὀρφανός": "orphan",
  "ἐμφανίζω": "to manifest, make known", "ὑπομιμνῄσκω": "to remind, bring to remembrance", "εἰρήνη": "peace", "δειλιάω": "to be afraid, be cowardly",
  // Chapter 15
  "ἄμπελος": "vine", "γεωργός": "farmer, vinedresser", "κλῆμα": "branch", "καθαίρω": "to prune, cleanse",
  "ξηραίνω": "to wither, dry up", "πῦρ": "fire", "γνωρίζω": "to make known", "μνημονεύω": "to remember",
  "πρόφασις": "excuse, pretext", "δωρεάν": "without cause, freely, for nothing",
  // Chapter 16
  "λατρεία": "service, worship", "προσφέρω": "to offer, present", "λύπη": "sorrow, grief", "δικαιοσύνη": "righteousness",
  "ὁδηγέω": "to guide, lead", "θρηνέω": "to lament, mourn", "λυπέω": "to grieve, cause sorrow", "τίκτω": "to give birth",
  "θλῖψις": "affliction, tribulation", "ἀπαγγέλλω": "to announce, report", "θαρσέω": "to take heart, be of good cheer", "νικάω": "to conquer, overcome",
  // Chapter 17
  "ἀπώλεια": "destruction, ruin", "καταβολή": "foundation, beginning",
  // Chapter 18
  "χείμαρρος": "brook, winter stream", "Κεδρών": "Kidron", "κῆπος": "garden", "πολλάκις": "often",
  "σπεῖρα": "cohort, band of soldiers", "φανός": "lantern", "λαμπάς": "torch, lamp", "ὅπλον": "weapon",
  "Ναζωραῖος": "Nazarene, of Nazareth", "μάχαιρα": "sword", "παίω": "to strike", "ἀποκόπτω": "to cut off",
  "ὠτάριον": "ear", "δεξιός": "right", "Μάλχος": "Malchus", "θήκη": "sheath",
  "ποτήριον": "cup", "χιλίαρχος": "commander (of a thousand)", "συλλαμβάνω": "to seize, arrest", "Ἅννας": "Annas",
  "πενθερός": "father-in-law", "συμβουλεύω": "to counsel, advise", "γνωστός": "known, acquaintance", "εἰσάγω": "to bring in",
  "παιδίσκη": "servant girl", "ἀνθρακιά": "charcoal fire", "ψῦχος": "cold", "θερμαίνω": "to warm",
  "παρίστημι": "to stand by", "ῥάπισμα": "a blow, slap", "κακῶς": "wrongly, badly", "κακός": "evil, wrong",
  "δέρω": "to strike, beat", "συγγενής": "relative, kinsman", "ὠτίον": "ear", "πραιτώριον": "praetorium, governor's headquarters",
  "πρωΐ": "early morning", "μιαίνω": "to defile", "Πιλᾶτος": "Pilate", "κατηγορία": "accusation, charge",
  "ἀγωνίζομαι": "to fight, struggle", "οὐκοῦν": "so then, therefore", "αἰτία": "cause, guilt, charge", "συνήθεια": "custom",
  "ἀπολύω": "to release", "βούλομαι": "to want, wish", "Βαραββᾶς": "Barabbas",
  // Chapter 19
  "μαστιγόω": "to flog, scourge", "στρατιώτης": "soldier", "πλέκω": "to weave, twist", "στέφανος": "crown",
  "ἄκανθα": "thorn", "πορφυροῦς": "purple", "περιβάλλω": "to clothe, put around", "φορέω": "to wear",
  "ἀκάνθινος": "of thorns", "σταυρόω": "to crucify", "Καῖσαρ": "Caesar", "ἀντιλέγω": "to oppose, speak against",
  "βῆμα": "judgment seat, tribunal", "λιθόστρωτος": "stone pavement", "Γαββαθά": "Gabbatha", "παρασκευή": "day of Preparation",
  "σταυρός": "cross", "κρανίον": "skull", "Γολγοθᾶ": "Golgotha", "τίτλος": "inscription, title",
  "ἀναγινώσκω": "to read", "Ῥωμαϊστί": "in Latin", "Ἑλληνιστί": "in Greek", "χιτών": "tunic",
  "ἄραφος": "seamless", "ὑφαντός": "woven", "σχίζω": "to tear, split", "λαγχάνω": "to cast lots, obtain by lot",
  "διαμερίζω": "to divide, distribute", "ἱματισμός": "clothing, garment", "κλῆρος": "lot", "Κλωπᾶς": "Clopas",
  "Μαγδαληνή": "Magdalene", "τελέω": "to finish, complete", "σκεῦος": "vessel, jar", "ὄξος": "sour wine, vinegar",
  "μεστός": "full", "σπόγγος": "sponge", "ὕσσωπος": "hyssop", "περιτίθημι": "to put around, place on",
  "στόμα": "mouth", "κλίνω": "to bow, incline", "κατάγνυμι": "to break", "σκέλος": "leg",
  "συσταυρόω": "to crucify together with", "λόγχη": "spear", "πλευρά": "side", "νύσσω": "to pierce, prick",
  "ὀστέον": "bone", "συντρίβω": "to break, crush", "ἕτερος": "another, other", "ἐκκεντέω": "to pierce",
  "Ἁριμαθαία": "Arimathea", "ἐπιτρέπω": "to permit, allow", "μίγμα": "mixture", "σμύρνα": "myrrh",
  "ἀλόη": "aloes", "ἑκατόν": "hundred", "ὀθόνιον": "linen cloth", "ἄρωμα": "spice, aromatic",
  "ἔθος": "custom", "ἐνταφιάζω": "to prepare for burial, bury",
  // Chapter 20
  "τρέχω": "to run", "προτρέχω": "to run ahead", "παρακύπτω": "to stoop and look in", "ἐντυλίσσω": "to wrap, fold up",
  "κηπουρός": "gardener", "ῥαββουνί": "Rabboni (my Teacher)", "ἅπτω": "to touch, cling to", "ἀγγέλλω": "to announce, report",
  "κλείω": "to shut, lock", "ἐμφυσάω": "to breathe on", "κρατέω": "to hold, retain", "τύπος": "mark, imprint",
  "ἧλος": "nail", "ἔσω": "inside, within", "ἄπιστος": "unbelieving, faithless", "πιστός": "believing, faithful",
  "ἐνώπιον": "before, in the presence of", "βιβλίον": "book, scroll",
  // Chapter 21
  "Ζεβεδαῖος": "Zebedee", "ἁλιεύω": "to fish", "πρωΐα": "morning, dawn", "αἰγιαλός": "shore, beach",
  "προσφάγιον": "fish, food eaten with bread", "δίκτυον": "net", "ἰσχύω": "to be able, be strong", "ἰχθύς": "fish",
  "ἐπενδύτης": "outer garment", "γυμνός": "naked, stripped for work", "μακράν": "far", "πῆχυς": "cubit",
  "σύρω": "to drag, haul", "ἀποβαίνω": "to step off, go out", "ἀριστάω": "to have breakfast, eat a meal", "τολμάω": "to dare",
  "ἐξετάζω": "to question, examine", "βόσκω": "to feed, tend", "ἀρνίον": "lamb", "ποιμαίνω": "to shepherd, tend",
  "προβάτιον": "sheep, little sheep", "νέος": "young", "ζωννύω": "to gird, dress", "γηράσκω": "to grow old",
  "ἐκτείνω": "to stretch out", "ἐπιστρέφω": "to turn around", "οἴομαι": "to suppose, think",

  // ── Matthew ──
  "βίβλος": "book, scroll", "γένεσις": "genesis, origin, birth", "Ἰσαάκ": "Isaac",
  "Φάρες": "Perez", "Ζάρα": "Zerah", "Θαμάρ": "Tamar", "Ἑσρώμ": "Hezron", "Ἀράμ": "Aram",
  "Ἀμιναδάβ": "Amminadab", "Ναασσών": "Nahshon", "Σαλμών": "Salmon", "Βόες": "Boaz",
  "Ῥαχάβ": "Rahab", "Ἰωβήδ": "Obed", "Ῥούθ": "Ruth", "Ἰεσσαί": "Jesse", "Οὐρίας": "Uriah",
  "Ῥοβοάμ": "Rehoboam", "Ἀβιά": "Abijah", "Ἀσάφ": "Asaph", "Ἰωσαφάτ": "Jehoshaphat",
  "Ἰωράμ": "Joram", "Ὀζίας": "Uzziah", "Ἰωαθάμ": "Jotham", "Ἀχάζ": "Ahaz",
  "Ἑζεκίας": "Hezekiah", "Μανασσῆ": "Manasseh", "Ἀμώς": "Amos", "Ἰωσίας": "Josiah",
  "Ἰεχονίας": "Jechoniah", "μετοικεσία": "deportation, exile", "Βαβυλών": "Babylon",
  "Σαλαθιήλ": "Shealtiel", "Ζοροβαβέλ": "Zerubbabel", "Ἀβιούδ": "Abiud", "Ἐλιακίμ": "Eliakim",
  "Ἀζώρ": "Azor", "Σαδώκ": "Zadok", "Ἀχίμ": "Achim", "Ἐλιούδ": "Eliud", "Ἐλεάζαρ": "Eleazar",
  "Ματθάν": "Matthan", "γενεά": "generation", "δεκατέσσαρες": "fourteen",
  "μνηστεύω": "to betroth", "γαστήρ": "womb, belly", "δειγματίζω": "to expose publicly, disgrace",
  "ἐνθυμέομαι": "to ponder, consider", "ὄναρ": "dream", "παρθένος": "virgin, maiden",
  "Ἐμμανουήλ": "Immanuel", "προστάσσω": "to command, direct",
  "Ἡρῴδης": "Herod", "μάγος": "magus, astrologer", "ἀνατολή": "rising; the east",
  "ἀστήρ": "star", "οὐδαμῶς": "by no means", "ἐλάχιστος": "least, smallest",
  "ἡγεμών": "ruler, governor", "ἡγέομαι": "to lead; to consider", "ἀκριβόω": "to ascertain exactly",
  "ἀκριβῶς": "carefully, exactly", "ἐπάν": "when, as soon as", "προάγω": "to go before, lead on",
  "οὗ": "where", "σφόδρα": "exceedingly", "θησαυρός": "treasure, chest", "δῶρον": "gift",
  "χρυσός": "gold", "λίβανος": "frankincense", "χρηματίζω": "to be divinely warned",
  "ἀνακάμπτω": "to return", "Αἴγυπτος": "Egypt", "τελευτή": "death, end",
  "ἐμπαίζω": "to mock, outwit", "θυμόω": "to be enraged", "λίαν": "greatly, exceedingly",
  "ἀναιρέω": "to kill, do away with", "ὅριον": "border, district", "διετής": "two years old",
  "Ἰερεμίας": "Jeremiah", "Ῥαμά": "Ramah", "κλαυθμός": "weeping", "ὀδυρμός": "lamentation",
  "Ῥαχήλ": "Rachel", "παρακαλέω": "to comfort, urge", "Ἀρχέλαος": "Archelaus",
  "βασιλεύω": "to reign", "κατοικέω": "to settle, dwell",
  "βαπτιστής": "Baptist, baptizer", "κηρύσσω": "to proclaim, preach", "μετανοέω": "to turn, change one's mind",
  "ἐγγίζω": "to draw near", "εὐθύς": "straight; at once", "τρίβος": "path", "ἔνδυμα": "garment",
  "κάμηλος": "camel", "ζώνη": "belt", "δερμάτινος": "of leather", "ὀσφῦς": "waist, loins",
  "ἀκρίς": "locust", "μέλι": "honey", "ἄγριος": "wild", "περίχωρος": "surrounding region",
  "ἐξομολογέω": "to confess", "Σαδδουκαῖος": "Sadducee", "βάπτισμα": "baptism",
  "γέννημα": "offspring, brood", "ἔχιδνα": "viper", "ὑποδείκνυμι": "to show, warn",
  "μετάνοια": "turning, change of mind", "ἀξίνη": "axe", "ῥίζα": "root", "δένδρον": "tree",
  "ἐκκόπτω": "to cut down", "ἰσχυρός": "strong, mighty", "ἱκανός": "sufficient, worthy",
  "πτύον": "winnowing-fork", "διακαθαρίζω": "to clear out, cleanse", "ἅλων": "threshing-floor",
  "ἀποθήκη": "barn, storehouse", "ἄχυρον": "chaff", "κατακαίω": "to burn up",
  "ἄσβεστος": "unquenchable", "διακωλύω": "to hinder, prevent", "πρέπω": "to be fitting",
  "ὡσεί": "like, as", "ἀγαπητός": "beloved", "εὐδοκέω": "to delight, be well pleased",
  "ἀνάγω": "to lead up", "νηστεύω": "to fast", "πτερύγιον": "pinnacle, little wing",
  "ἐκπειράζω": "to put to the test", "ὑψηλός": "high", "λατρεύω": "to serve, worship",
  "παραθαλάσσιος": "by the sea", "Ζαβουλών": "Zebulun", "Νεφθαλίμ": "Naphtali",
  "σκιά": "shadow", "ἀνατέλλω": "to rise, dawn", "ἀμφίβληστρον": "casting-net",
  "ἁλιεύς": "fisherman", "προβαίνω": "to go on, advance", "Ἰάκωβος": "James (Jacob)",
  "καταρτίζω": "to mend, prepare", "περιάγω": "to go about", "εὐαγγέλιον": "good news, gospel",
  "νόσος": "disease", "μαλακία": "infirmity, ailment", "Συρία": "Syria",
  "ποικίλος": "various", "βάσανος": "torment", "συνέχω": "to afflict, seize",
  "σεληνιάζομαι": "to be moonstruck, epileptic", "παραλυτικός": "paralytic", "Δεκάπολις": "Decapolis",
  "πενθέω": "to mourn", "πραΰς": "meek, gentle", "κληρονομέω": "to inherit",
  "ἐλεήμων": "merciful", "ἐλεέω": "to show mercy", "εἰρηνοποιός": "peacemaker",
  "ἕνεκα": "for the sake of", "ὀνειδίζω": "to revile, reproach", "ψεύδομαι": "to lie",
  "ἅλας": "salt", "μωραίνω": "to lose taste, become foolish", "ἁλίζω": "to salt",
  "καταπατέω": "to trample", "μόδιος": "measuring-basket", "λυχνία": "lampstand",
  "λάμπω": "to shine", "νομίζω": "to suppose", "καταλύω": "to tear down, abolish",
  "παρέρχομαι": "to pass away", "ἰῶτα": "iota, yod", "κεραία": "stroke, horn (of a letter)",
  "ἀρχαῖος": "ancient", "φονεύω": "to murder", "ἔνοχος": "liable, guilty",
  "ὀργίζω": "to be angry", "ῥακά": "raka (empty-head)", "μωρός": "fool",
  "γέεννα": "Gehenna", "θυσιαστήριον": "altar", "διαλλάσσω": "to be reconciled",
  "εὐνοέω": "to come to terms, be well-disposed", "ἀντίδικος": "accuser, opponent at law",
  "κριτής": "judge", "ἀποδίδωμι": "to pay back, render", "κοδράντης": "quadrans (small coin)",
  "ἐπιθυμέω": "to covet, desire", "ἐξαιρέω": "to tear out", "μέλος": "member, limb",
  "ἀποστάσιον": "certificate of divorce", "παρεκτός": "except", "γαμέω": "to marry",
  "μοιχάω": "to commit adultery", "ἐπιορκέω": "to swear falsely", "ὅρκος": "oath",
  "ὀμνύω": "to swear", "ὅλως": "at all", "μήτε": "neither, nor", "θρόνος": "throne",
  "ὑποπόδιον": "footstool", "μέλας": "black", "ὀδούς": "tooth",
  "ἀνθίστημι": "to resist, oppose", "ῥαπίζω": "to strike, slap", "σιαγών": "cheek",
  "ἀγγαρεύω": "to press into service", "μίλιον": "mile", "δανίζω": "to lend, borrow",
  "ἀποστρέφω": "to turn away", "ἐχθρός": "enemy", "προσεύχομαι": "to pray",
  "ἥλιος": "sun", "βρέχω": "to rain", "ἄδικος": "unjust", "τελώνης": "tax-collector",
  "ἀσπάζομαι": "to greet", "ἐθνικός": "gentile", "τέλειος": "whole, complete", "οὐράνιος": "heavenly",
  "προσέχω": "to take care, beware", "εἰ δὲ μή": "otherwise", "ἐλεημοσύνη": "alms",
  "σαλπίζω": "to sound a trumpet", "ὑποκριτής": "hypocrite, actor", "ῥύμη": "street, lane",
  "ἀπέχω": "to have in full", "ἀριστερός": "left (hand)", "γωνία": "corner",
  "πλατύς": "broad street, square", "ταμεῖον": "storeroom, inner room", "βατταλογέω": "to babble",
  "πολυλογία": "much speaking", "εἰσακούω": "to hear, listen to", "ὁμοιόω": "to be like, liken",
  "ἐπιούσιος": "for the coming day (daily)", "σήμερον": "today", "ὀφείλημα": "debt",
  "ὀφειλέτης": "debtor", "εἰσφέρω": "to bring into", "πειρασμός": "testing, temptation",
  "ῥύομαι": "to rescue, deliver", "παράπτωμα": "misstep, trespass", "σκυθρωπός": "gloomy-faced",
  "ἀφανίζω": "to disfigure; to consume", "πρόσωπον": "face", "κρυφαῖος": "hidden",
  "θησαυρίζω": "to store up", "σής": "moth", "διορύσσω": "to dig through, break in",
  "ἁπλοῦς": "single, sound", "φωτεινός": "full of light", "σκοτεινός": "dark",
  "πόσος": "how great, how much", "ἀντέχω": "to hold to", "καταφρονέω": "to despise",
  "μαμωνᾶς": "Mammon, wealth", "μεριμνάω": "to be anxious", "ἐνδύω": "to clothe, put on",
  "πετεινός": "bird", "τρέφω": "to feed", "διαφέρω": "to be worth more, differ",
  "προστίθημι": "to add", "καταμανθάνω": "to study, observe", "κρίνον": "lily",
  "ἀγρός": "field", "νήθω": "to spin", "αὔριον": "tomorrow", "κλίβανος": "oven",
  "ἀμφιέννυμι": "to clothe", "ὀλιγόπιστος": "of little faith", "ἐπιζητέω": "to pursue, seek after",
  "χρῄζω": "to need", "ἀρκετός": "sufficient", "κακία": "trouble, evil",
  "μετρέω": "to measure", "κάρφος": "speck, splinter", "δοκός": "log, beam",
  "κατανοέω": "to notice, consider", "διαβλέπω": "to see clearly", "κύων": "dog",
  "μαργαρίτης": "pearl", "χοῖρος": "pig, swine", "ῥήγνυμι": "to tear, break",
  "κρούω": "to knock", "ἐπιδίδωμι": "to hand over, give", "δόμα": "gift",
  "στενός": "narrow", "πύλη": "gate", "εὐρύχωρος": "broad, spacious",
  "ἀπάγω": "to lead away", "θλίβω": "to press, afflict", "ὀλίγος": "few, little",
  "ψευδοπροφήτης": "false prophet", "ἔσωθεν": "within, inside", "ἅρπαξ": "ravenous, rapacious",
  "ἐπιγινώσκω": "to recognize, know", "συλλέγω": "to gather", "σταφυλή": "grape",
  "τρίβολος": "thistle", "σῦκον": "fig", "σαπρός": "rotten, bad", "ἄρα": "then, therefore",
  "δύναμις": "power, mighty work", "ἀποχωρέω": "to go away", "ἀνομία": "lawlessness",
  "φρόνιμος": "sensible, prudent", "πέτρα": "rock", "βροχή": "rain",
  "προσπίπτω": "to fall upon", "θεμελιόω": "to found, ground", "ἄμμος": "sand",
  "πτῶσις": "fall", "ἐκπλήσσω": "to astonish",
  // Matthew 8
  "λεπρός": "leper", "καθαρίζω": "to cleanse, make clean", "λέπρα": "leprosy",
  "μαρτύριον": "testimony, witness", "ἑκατοντάρχης": "centurion", "δεινῶς": "terribly",
  "βασανίζω": "to torment", "στέγη": "roof", "πίστις": "faith, trust",
  "δυσμή": "west, setting (of sun)", "ἀνακλίνω": "to recline (at table)", "ἐξώτερος": "outer",
  "βρυγμός": "gnashing", "πενθερά": "mother-in-law", "πυρέσσω": "to have a fever",
  "κελεύω": "to command, order", "ἀλώπηξ": "fox", "φωλεός": "den, hole",
  "κατασκήνωσις": "nest, roost", "θάπτω": "to bury", "σεισμός": "shaking, storm; earthquake",
  "καλύπτω": "to cover", "κῦμα": "wave", "καθεύδω": "to sleep",
  "δειλός": "afraid, cowardly", "ἐπιτιμάω": "to rebuke", "γαλήνη": "calm",
  "ποταπός": "what sort of", "ὑπακούω": "to obey", "Γαδαρηνός": "Gadarene",
  "χαλεπός": "violent, fierce", "ἀγέλη": "herd", "δαίμων": "demon",
  "ὁρμάω": "to rush", "κρημνός": "steep bank, cliff",
  // Matthew 9
  "διαπεράω": "to cross over", "κλίνη": "bed, couch", "ἐνθύμησις": "thought, inner reflection",
  "εὔκοπος": "easy", "τελώνιον": "tax booth, toll station", "Μαθθαῖος": "Matthew",
  "συνανάκειμαι": "to recline (at table) with", "ἰατρός": "physician, doctor", "ἔλεος": "mercy",
  "θυσία": "sacrifice", "νυμφών": "bridal chamber", "ἀπαίρω": "to take away",
  "ἐπίβλημα": "patch", "ῥάκος": "piece of cloth", "ἄγναφος": "unshrunk, new",
  "παλαιός": "old", "ἀσκός": "wineskin", "ἀμφότεροι": "both",
  "συντηρέω": "to preserve, keep safe", "αἱμορροέω": "to suffer a flow of blood", "ὄπισθεν": "behind",
  "κράσπεδον": "fringe, tassel", "αὐλητής": "flute player", "θορυβέω": "to make an uproar",
  "κοράσιον": "girl", "καταγελάω": "to laugh at, deride", "φήμη": "report, fame",
  "διαφημίζω": "to spread abroad", "κωφός": "mute, deaf", "σπλαγχνίζομαι": "to be moved with compassion",
  "σκύλλω": "to harass, weary", "ῥίπτω": "to throw down, cast", "ἐργάτης": "worker, labourer",
  "δέομαι": "to beg, plead",
  // Matthew 10
  "προσκαλέω": "to call to oneself", "ἀκάθαρτος": "unclean", "Βαρθολομαῖος": "Bartholomew",
  "Ἁλφαῖος": "Alphaeus", "Θαδδαῖος": "Thaddaeus", "Καναναῖος": "Cananaean, zealot",
  "παραγγέλλω": "to instruct, command", "κτάομαι": "to acquire, get", "ἄργυρος": "silver",
  "χαλκός": "copper, bronze", "πήρα": "bag, knapsack", "ῥάβδος": "staff",
  "ἐκτινάσσω": "to shake off", "κονιορτός": "dust", "ἀνεκτός": "bearable, tolerable",
  "Σόδομα": "Sodom", "Γόμορρα": "Gomorrah", "ἀκέραιος": "innocent, unmixed",
  "ἐπανίστημι": "to rise up against", "θανατόω": "to put to death", "ὑπομένω": "to endure",
  "οἰκοδεσπότης": "master of the house", "Βεελζεβούλ": "Beelzebul", "ἐπικαλέω": "to call, name",
  "οἰκιακός": "member of a household", "οὖς": "ear", "δῶμα": "housetop, roof",
  "στρουθίον": "sparrow", "ἀσσάριον": "assarion (small coin)", "ἄνευ": "without",
  "ἀριθμέω": "to number, count", "διχάζω": "to divide, set against", "ποτίζω": "to give to drink",
  "ψυχρός": "cold",
  // Matthew 11
  "διατάσσω": "to instruct, give orders", "δεσμωτήριον": "prison", "προσδοκάω": "to wait for, expect",
  "εὐαγγελίζω": "to bring good news", "κάλαμος": "reed", "σαλεύω": "to shake",
  "μαλακός": "soft", "κατασκευάζω": "to prepare", "γεννητός": "born",
  "βιάζω": "to force; to suffer violence", "βιαστής": "violent man", "ἀγορά": "marketplace",
  "προσφωνέω": "to call out to", "αὐλέω": "to play the flute", "ὀρχέομαι": "to dance",
  "κόπτω": "to mourn, beat the breast", "φάγος": "glutton", "οἰνοπότης": "wine-drinker",
  "δικαιόω": "to justify, vindicate", "σοφία": "wisdom", "οὐαί": "woe",
  "Χοραζίν": "Chorazin", "Τύρος": "Tyre", "Σιδών": "Sidon",
  "πάλαι": "long ago", "σάκκος": "sackcloth", "σποδός": "ashes",
  "ᾅδης": "Hades", "μέχρι": "until", "σοφός": "wise",
  "συνετός": "understanding, intelligent", "νήπιος": "infant, child", "εὐδοκία": "good pleasure",
  "φορτίζω": "to load, burden", "ἀναπαύω": "to give rest", "ζυγός": "yoke",
  "ταπεινός": "lowly, humble", "ἀνάπαυσις": "rest", "χρηστός": "kind, good",
  "φορτίον": "burden, load", "ἐλαφρός": "light (in weight)",
  // Matthew 12
  "σπόριμος": "grainfield", "τίλλω": "to pluck", "στάχυς": "head of grain",
  "πρόθεσις": "setting forth; (bread of) the Presence", "βεβηλόω": "to profane", "ἀναίτιος": "guiltless",
  "καταδικάζω": "to condemn", "ἐμπίπτω": "to fall into", "βόθυνος": "pit",
  "ἀποκαθιστάνω": "to restore", "συμβούλιον": "counsel, plot", "φανερός": "known, manifest",
  "αἱρετίζω": "to choose", "ἐρίζω": "to quarrel", "λίνον": "wick, flax",
  "τύφω": "to smoulder", "σβέννυμι": "to quench, extinguish", "νῖκος": "victory",
  "ἐξίστημι": "to be amazed", "μερίζω": "to divide", "ἐρημόω": "to lay waste",
  "φθάνω": "to arrive, come upon", "διαρπάζω": "to plunder", "περίσσευμα": "overflow, abundance",
  "ἀργός": "idle, useless", "μοιχαλίς": "adulterous", "Ἰωνᾶς": "Jonah",
  "κῆτος": "sea monster", "Νινευίτης": "Ninevite", "κήρυγμα": "proclamation",
  "βασίλισσα": "queen", "νότος": "south", "πέρας": "end, limit",
  "ἄνυδρος": "waterless", "ὅθεν": "from where", "σχολάζω": "to stand empty",
  "σαρόω": "to sweep", "κοσμέω": "to adorn, decorate", "ἑπτά": "seven",
  // Matthew 13
  "παραβολή": "parable", "πετρώδης": "rocky", "ἐξανατέλλω": "to spring up",
  "βάθος": "depth", "καυματίζω": "to scorch", "ἀποπνίγω": "to choke",
  "ἑξήκοντα": "sixty", "μυστήριον": "mystery", "συνίημι": "to understand",
  "ἀναπληρόω": "to fulfil", "προφητεία": "prophecy", "παχύνω": "to grow dull, thicken",
  "βαρέως": "heavily, with difficulty", "καμμύω": "to close (the eyes)", "πρόσκαιρος": "lasting a season, temporary",
  "διωγμός": "persecution", "μέριμνα": "anxiety, care", "ἀπάτη": "deceit",
  "πλοῦτος": "wealth, riches", "συμπνίγω": "to choke, crowd out", "ἄκαρπος": "fruitless",
  "δή": "indeed", "καρποφορέω": "to bear fruit", "παρατίθημι": "to set before",
  "ἐπισπείρω": "to sow over", "ζιζάνιον": "weed, darnel", "βλαστάνω": "to sprout",
  "ἐκριζόω": "to uproot", "ἅμα": "together with", "συναυξάνω": "to grow together",
  "θεριστής": "reaper", "δέσμη": "bundle", "σίναπι": "mustard",
  "λάχανον": "garden plant", "κατασκηνόω": "to nest, dwell", "κλάδος": "branch",
  "ζύμη": "leaven", "ἐγκρύπτω": "to hide in", "ἄλευρον": "flour",
  "σάτον": "saton (measure)", "ζυμόω": "to leaven", "ἐρεύγομαι": "to utter",
  "διασαφέω": "to explain", "συντέλεια": "completion, end", "σκάνδαλον": "stumbling-block",
  "κάμινος": "furnace", "ἐκλάμπω": "to shine out", "ἔμπορος": "merchant",
  "σαγήνη": "dragnet", "γένος": "kind, sort", "ἀναβιβάζω": "to haul up",
  "ἄγγος": "vessel", "ἀφορίζω": "to separate", "μαθητεύω": "to disciple, make a disciple",
  "μεταίρω": "to move on", "τέκτων": "carpenter, builder", "ἄτιμος": "without honour",
  "ἀπιστία": "unbelief",
  // Matthew 14
  "τετραάρχης": "tetrarch", "ἐνεργέω": "to be at work", "ἀποτίθημι": "to put away",
  "Ἡρῳδιάς": "Herodias", "γενέσια": "birthday celebration", "ἀρέσκω": "to please",
  "προβιβάζω": "to prompt, urge forward", "πίναξ": "platter", "ἀποκεφαλίζω": "to behead",
  "πτῶμα": "corpse", "πεζῇ": "on foot", "ἄρρωστος": "sick, ill",
  "κλάω": "to break", "ἀναγκάζω": "to compel, make", "ἐναντίος": "against, contrary",
  "τέταρτος": "fourth", "φάντασμα": "ghost, apparition", "καταποντίζω": "to sink",
  "ἐπιλαμβάνομαι": "to take hold of", "διστάζω": "to doubt, waver", "κοπάζω": "to drop, cease",
  "Γεννησαρέτ": "Gennesaret", "διασῴζω": "to bring safely through",
  // Matthew 15
  "παραβαίνω": "to transgress", "παράδοσις": "tradition", "κακολογέω": "to speak evil of",
  "ἀκυρόω": "to void, annul", "χεῖλος": "lip", "πόρρω": "far away",
  "μάτην": "in vain", "σέβω": "to worship", "διδασκαλία": "teaching",
  "ἔνταλμα": "commandment, precept", "κοινόω": "to defile, make common", "φυτεία": "plant",
  "φυτεύω": "to plant", "ὁδηγός": "guide", "φράζω": "to explain",
  "ἀκμήν": "still, even now", "ἀσύνετος": "without understanding", "εἰσπορεύομαι": "to enter",
  "ἀφεδρών": "latrine", "διαλογισμός": "design, reasoning", "φόνος": "murder",
  "κλοπή": "theft", "ψευδομαρτυρία": "false testimony", "ἄνιπτος": "unwashed",
  "Χαναναῖος": "Canaanite", "βοηθέω": "to help", "κυνάριον": "little dog, house-dog",
  "ψιχίον": "crumb", "ὦ": "O", "κυλλός": "maimed, crippled",
  "προσμένω": "to stay with, remain", "νῆστις": "fasting, hungry", "ἐκλύω": "to collapse, give out",
  "ἐρημία": "desert, wilderness", "ἰχθύδιον": "small fish", "σπυρίς": "basket, hamper",
  "τετρακισχίλιοι": "four thousand", "Μαγαδάν": "Magadan",
  // Matthew 16
  "ἐπιδείκνυμι": "to show, display", "εὐδία": "fair weather", "πυρράζω": "to be red",
  "στυγνάζω": "to be lowering, gloomy", "διακρίνω": "to discern", "ἐπιλανθάνομαι": "to forget",
  "διαλογίζομαι": "to reason, discuss", "Καισάρεια": "Caesarea", "Βαριωνᾶ": "Bar-Jonah",
  "ἐκκλησία": "church, assembly", "κατισχύω": "to prevail against", "κλείς": "key",
  "πάσχω": "to suffer", "προσλαμβάνω": "to take aside", "ἵλεως": "merciful; far be it",
  "φρονέω": "to be minded, think", "ἀπαρνέομαι": "to deny", "κερδαίνω": "to gain",
  "ζημιόω": "to forfeit, lose", "ἀντάλλαγμα": "exchange, price", "πρᾶξις": "deed, practice",
  // Matthew 17
  "ἀναφέρω": "to lead up", "μεταμορφόω": "to transfigure, transform", "συλλαλέω": "to talk with",
  "σκηνή": "tent, booth", "νεφέλη": "cloud", "ἐπισκιάζω": "to overshadow",
  "ὅραμα": "vision", "γονυπετέω": "to kneel", "διαστρέφω": "to twist, pervert",
  "ἀνέχω": "to bear with, endure", "ὀλιγοπιστία": "little faith", "ἔνθεν": "from here",
  "ἀδυνατέω": "to be impossible", "συστρέφω": "to gather", "δίδραχμον": "double-drachma (temple tax)",
  "προφθάνω": "to anticipate, speak first", "κῆνσος": "tax, tribute", "ἄγκιστρον": "fish-hook",
  "στατήρ": "stater (coin)",
  // Matthew 18
  "ἆρα": "then, so", "ταπεινόω": "to lower, humble", "κρεμάννυμι": "to hang",
  "μύλος": "millstone", "ὀνικός": "of a donkey", "τράχηλος": "neck",
  "πέλαγος": "open sea, depth", "ἀνάγκη": "necessity", "μονόφθαλμος": "one-eyed",
  "ἐνενήκοντα": "ninety", "ἐννέα": "nine", "μάρτυς": "witness",
  "παρακούω": "to refuse to hear", "συμφωνέω": "to agree", "πρᾶγμα": "matter, thing",
  "ποσάκις": "how often", "ἑπτάκις": "seven times", "ἑβδομηκοντάκις": "seventy times",
  "συναίρω": "to settle (accounts)", "προσάγω": "to bring to", "μύριοι": "ten thousand, countless",
  "τάλαντον": "talent (large sum)", "μακροθυμέω": "to be patient", "δάνειον": "loan, debt",
  "σύνδουλος": "fellow slave", "πνίγω": "to choke", "ὀφειλή": "debt",
  "βασανιστής": "torturer, jailer",
  // Matthew 19
  "κτίζω": "to create", "ἄρσην": "male", "θῆλυς": "female",
  "κολλάω": "to join, cleave to", "συζεύγνυμι": "to yoke together", "χωρίζω": "to separate",
  "σκληροκαρδία": "hardness of heart", "εὐνοῦχος": "eunuch", "εὐνουχίζω": "to make a eunuch",
  "κωλύω": "to hinder, forbid", "ψευδομαρτυρέω": "to bear false witness", "νεανίσκος": "young man",
  "ὑπάρχω": "to possess; belongings", "κτῆμα": "possession", "πλούσιος": "rich",
  "δυσκόλως": "with difficulty", "τρῆμα": "eye (of needle), hole", "ῥαφίς": "needle",
  "ἀδύνατος": "impossible", "δυνατός": "possible, able", "παλιγγενεσία": "regeneration, rebirth",
  "φυλή": "tribe", "πολλαπλασίων": "many times more",
  // Matthew 20
  "μισθόω": "to hire", "ἀμπελών": "vineyard", "ἔνατος": "ninth",
  "ὡσαύτως": "likewise", "ἑνδέκατος": "eleventh", "ἐπίτροπος": "steward",
  "βάρος": "burden", "καύσων": "scorching heat", "ἑταῖρος": "friend, comrade",
  "ἀδικέω": "to wrong, do injustice", "εὐώνυμος": "left (side)", "δέκα": "ten",
  "ἀγανακτέω": "to be indignant", "κατακυριεύω": "to lord it over", "κατεξουσιάζω": "to wield authority over",
  "λύτρον": "ransom", "Ἰεριχώ": "Jericho", "σιωπάω": "to be silent",
  "ὄμμα": "eye",
  // Matthew 21
  "Βηθφαγή": "Bethphage", "κατέναντι": "opposite", "ἐπιβαίνω": "to mount, ride on",
  "ὑποζύγιον": "beast of burden", "συντάσσω": "to direct, order", "ἐπικαθίζω": "to sit on",
  "στρωννύω": "to spread", "ὕψιστος": "highest", "σείω": "to shake",
  "καταστρέφω": "to overturn", "καθέδρα": "seat, chair", "προσευχή": "prayer",
  "θαυμάσιος": "wonderful thing", "θηλάζω": "to nurse, suckle", "αἶνος": "praise",
  "αὐλίζομαι": "to spend the night", "ἐπανάγω": "to return", "φύλλον": "leaf",
  "παραχρῆμα": "at once, immediately", "μεταμέλομαι": "to repent, change one's mind", "πόρνη": "prostitute",
  "φραγμός": "fence, hedge", "ὀρύσσω": "to dig", "ληνός": "winepress",
  "πύργος": "tower", "ἐκδίδωμι": "to lease out", "ἀποδημέω": "to go abroad",
  "ἐντρέπω": "to respect", "κληρονόμος": "heir", "κληρονομία": "inheritance",
  "ἀποδοκιμάζω": "to reject", "συνθλάω": "to break in pieces", "λικμάω": "to crush, scatter",
  // Matthew 22
  "ἄριστον": "dinner, feast", "ταῦρος": "bull", "σιτιστός": "fatling, fattened animal",
  "ἀμελέω": "to pay no attention", "ἐμπορία": "business, trade", "λοιπός": "the rest",
  "ὑβρίζω": "to abuse, insult", "στράτευμα": "army", "φονεύς": "murderer",
  "ἐμπίπρημι": "to burn", "διέξοδος": "crossroads", "πίμπλημι": "to fill",
  "φιμόω": "to silence, muzzle", "κλητός": "called, invited", "ἐκλεκτός": "chosen",
  "παγιδεύω": "to trap, ensnare", "Ἡρῳδιανοί": "Herodians", "πονηρία": "malice, wickedness",
  "νόμισμα": "coin", "εἰκών": "image", "ἐπιγραφή": "inscription",
  "ἐπιγαμβρεύω": "to marry (as next of kin)", "γαμίζω": "to give in marriage", "νομικός": "lawyer",
  "διάνοια": "mind, understanding",
  // Matthew 23
  "δεσμεύω": "to bind", "βαρύς": "heavy, weighty", "ὦμος": "shoulder",
  "κινέω": "to move", "πλατύνω": "to broaden", "φυλακτήριον": "phylactery",
  "μεγαλύνω": "to enlarge", "πρωτοκλισία": "place of honour", "πρωτοκαθεδρία": "first seat",
  "ἀσπασμός": "greeting", "καθηγητής": "instructor, guide", "προσήλυτος": "proselyte, convert",
  "διπλοῦς": "double, twice", "ἀποδεκατόω": "to tithe", "ἡδύοσμον": "mint",
  "ἄνηθον": "dill", "κύμινον": "cumin", "διϋλίζω": "to strain out",
  "κώνωψ": "gnat", "καταπίνω": "to swallow", "ἔξωθεν": "outside, outwardly",
  "παροψίς": "dish, plate", "γέμω": "to be full", "ἁρπαγή": "plunder, greed",
  "ἀκρασία": "self-indulgence", "ἐντός": "inside", "ἐκτός": "outside",
  "παρομοιάζω": "to be like", "τάφος": "tomb, grave", "κονιάω": "to whitewash",
  "ὡραῖος": "beautiful", "ἀκαθαρσία": "uncleanness", "ὑπόκρισις": "hypocrisy",
  "κοινωνός": "partner, sharer", "Ἅβελ": "Abel", "Ζαχαρίας": "Zechariah",
  "Βαραχίας": "Barachiah", "ἐπισυνάγω": "to gather together", "τρόπος": "way, manner",
  "ὄρνις": "hen, bird", "νοσσίον": "chick", "πτέρυξ": "wing",
  // Matthew 24
  "οἰκοδομή": "building", "παρουσία": "coming, arrival", "πόλεμος": "war",
  "θροέω": "to alarm, frighten", "λιμός": "famine", "ὠδίν": "birth pang",
  "πληθύνω": "to multiply", "ψύχω": "to grow cold", "οἰκουμένη": "inhabited world",
  "βδέλυγμα": "abomination", "ἐρήμωσις": "desolation", "Δανιήλ": "Daniel",
  "φυγή": "flight", "οἷος": "such as", "κολοβόω": "to shorten, cut short",
  "ψευδόχριστος": "false christ", "προλέγω": "to tell beforehand", "ἀστραπή": "lightning",
  "ἀετός": "vulture, eagle", "σκοτίζω": "to darken", "σελήνη": "moon",
  "φέγγος": "light, radiance", "σάλπιγξ": "trumpet", "ἄκρον": "end, extremity",
  "ἁπαλός": "tender", "ἐκφύω": "to put out (leaves)", "θέρος": "summer",
  "Νῶε": "Noah", "κατακλυσμός": "flood", "ἄχρι": "until",
  "κιβωτός": "ark", "ἀλήθω": "to grind", "γρηγορέω": "to watch, stay awake",
  "ἐάω": "to let, allow", "καθίστημι": "to set over, appoint", "οἰκετεία": "household",
  "χρονίζω": "to delay", "τύπτω": "to beat, strike", "διχοτομέω": "to cut in two",
};
